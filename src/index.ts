...
async function main(): Promise<void> {
	if (config.mode === 'stdio') {
		...
		// In-memory store for pending OAuth flows and generated codes
		const pendingFlows = new Map<string, { redirectUri: string; state: string }>();
		...
		app.get('/callback', async (req: Request, res: Response) => {
			...
			try {
				...
				// Check if this was initiated by Claude's OAuth flow
				if (state && pendingFlows.has(state)) {
					const flow = pendingFlows.get(state)!;
					pendingFlows.delete(state);

					// Generate a code for Claude
					const claudeCode = crypto.randomUUID();
					generatedCodes.set(claudeCode, Date.now());

					// Redirect back to Claude with the code
					const redirectUrl = new URL(flow.redirectUri);
					redirectUrl.searchParams.set('code', claudeCode);
		...
		// OAuth authorize endpoint — saves Claude's redirect info, then redirects to Whoop OAuth
		app.get('/oauth/authorize', (req: Request, res: Response) => {
			const scopes = ['read:profile', 'read:body_measurement', 'read:cycles', 'read:recovery', 'read:sleep', 'read:workout', 'offline'];
			const claudeRedirectUri = req.query.redirect_uri as string | undefined;
			const claudeState = req.query.state as string | undefined;
			const ourState = crypto.randomUUID();

			// Save Claude's redirect info so /callback can redirect back
			if (claudeRedirectUri && claudeState) {
				pendingFlows.set(ourState, { redirectUri: claudeRedirectUri, state: claudeState });
			}
		...
		// OAuth token endpoint — handles codes generated by our server for Claude
		app.post('/oauth/token', async (req: Request, res: Response) => {
			try {
				...
				if (grantType === 'authorization_code' && code) {
					// Check if this is a code we generated for Claude
					...
					// Otherwise try exchanging with Whoop directly
					const tokens = await client.exchangeCodeForTokens(code);
					db.saveTokens(tokens);
					sync.syncDays(90).catch(() => {});
					res.json({
				...
				} else if (grantType === 'refresh_token' && reqRefreshToken) {
					// Return a fresh token for Claude
					res.json({
